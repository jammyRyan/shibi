...
module.exports = (app) => {
	const WS_URL = 'wss://www.btcgateway.pro/ws';
	...
	const getFrom = async ()=>{
		let last = await app.service('kline-btc').find({
			query:{
				$limit:1,
				$sort:{
					id:-1
		...
		if(last.data[0]){
			from = Number(last.data[0].id) + 300;
		}
		else{
			from = 1559318400;
		}
		...
		console.log(from);
		console.log(getTo(from));
		return from;
	}

	const getTo = from =>{
		return Number(moment.unix(from).add(5,'d').subtract(5,'m').format('X'));
		// return from + 432000 - 300;
	}

	const handle =async (kline)=> {
		//console.log(app);
		const klineSv = app.service('kline-btc');
	...
	return function huobiWs(req, res, next) {
		// let from = getFrom();
		// let from = 1559318400;
		// let to = getTo(from);
		start();
